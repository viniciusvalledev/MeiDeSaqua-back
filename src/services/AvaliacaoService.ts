...
class AvaliacaoService {
    ...
    public async submeterAvaliacao(dadosAvaliacao: any, usuarioLogadoId: number) {
        ...
        if (!estabelecimento) {
            throw new Error(`Estabelecimento não encontrado com o ID: ${estabelecimentoId}`);
        }
    ...
    public async atualizarAvaliacao(avaliacaoId: number, dadosAvaliacao: any, usuarioLogadoId: number) {
        ...
        if (dadosAvaliacao.nota && (dadosAvaliacao.nota < 1 || dadosAvaliacao.nota > 5)) {
            throw new Error("A nota da avaliação deve estar entre 1 e 5.");
        }
    ...
    public async listarPorEstabelecimentoDTO(estabelecimentoId: number) {
        return Avaliacao.findAll({
            ...
            // Inclui os dados do utilizador, mas exclui a senha
            include: [{
                model: Usuario,
