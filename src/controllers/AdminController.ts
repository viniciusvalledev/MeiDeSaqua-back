...
import Estabelecimento, { StatusEstabelecimento } from "../entities/Estabelecimento.entity"; 
...
export class AdminController {
  ...
  static async getPending(req: Request, res: Response) {
    try {
      const cadastros = await Estabelecimento.findAll({ where: { status: StatusEstabelecimento.PENDENTE_APROVACAO } });
      const atualizacoes = await Estabelecimento.findAll({ where: { status: StatusEstabelecimento.PENDENTE_ATUALIZACAO } });
      const exclusoes = await Estabelecimento.findAll({ where: { status: StatusEstabelecimento.PENDENTE_EXCLUSAO } });

      return res.json({ cadastros, atualizacoes, exclusoes });
    } catch (error) {
        console.error(error)
      return res.status(500).json({ message: "Erro ao buscar solicitações pendentes." });
    }
  }
