...
class EstabelecimentoController {
  ...
  private _handleError = (error: any, res: Response): Response => {
    ...
    if (error.name === 'SequelizeDatabaseError' && error.message.includes('Data too long for column')) {
      let friendlyMessage = "Um dos campos de texto excedeu o limite de caracteres.";
      if (error.message.includes("'descricao_diferencial'")) {
        friendlyMessage = "O campo 'Diferencial' excedeu o limite de 130 caracteres.";
      } else if (error.message.includes("'descricao'")) {
        friendlyMessage = "O campo 'Descrição' excedeu o limite de 500 caracteres.";
      }
      return res.status(400).json({ message: friendlyMessage });
    }

    if (error.name === 'SequelizeUniqueConstraintError') {
      return res.status(400).json({ message: "O CNPJ informado já está cadastrado no sistema." });
    }
  ...
  private _prepareDadosCompletos = (req: Request): any => {
    ...
    const produtosPaths = arquivos["produtos"]?.map((file) => file.path.replace(/\\/g, "/")) || [];
  ...
  public cadastrar = async (req: Request, res: Response): Promise<Response> => {
    try {
      ...
      const novoEstabelecimento = await EstabelecimentoService.cadastrarEstabelecimentoComImagens(dadosCompletos);
  ...
  public solicitarAtualizacao = async (req: Request, res: Response): Promise<Response> => {
    try {
      ...
      const dadosCompletos = this._prepareDadosCompletos(req);
      const estabelecimento = await EstabelecimentoService.solicitarAtualizacaoPorCnpj(cnpj, dadosCompletos);
  ...
  public buscarPorId = async (req: Request, res: Response): Promise<Response> => {
    try {
      ...
      if (!estabelecimento) {
        return res.status(404).json({ message: "Estabelecimento não encontrado ou não está ativo." });
      }
    ...
    try {
      ...
      if (typeof ativo !== "boolean") {
        return res.status(400).json({
          message: "O corpo da requisição deve conter a chave 'ativo' com um valor booleano (true/false).",
        });
      }
